...
- 인증/환경설정
  - [ ] options 페이지에서 이메일/패스워드 로그인·세션 유지 구현
  - [ ] `.env`/config 주입으로 SUPABASE_URL/ANON_KEY 사용 (service role 금지)
- 메시징·도메인
  - [ ] background ↔ popup ↔ content 메시지 라우팅 (현재 stub)
  - [ ] domainExtractor 유틸 구현 및 popup에 적용 (현 popup.ts TODO)
- 워크플로우 목록 & 실행
  - [ ] 현재 도메인 식별 → workflows 리스트 fetch(auth) (popup.ts TODO)
  - [ ] Free/Pro 필터링 및 Pro 잠금 표시 (features.json `ext-free-pro-filtering`)
  - [ ] 실행 시 COMET DOM에 프롬프트 삽입 + submit (content.ts + cometDom)
- 로깅 연동
  - [ ] 실행 클릭 시 logs status='started' insert
  - [ ] final-goal 감지 후 status/meta 업데이트 (turns/duration_ms/final_goal_text)
  - [ ] 오류/타임아웃 규칙 적용
- UI/UX 최소 요구
  - [ ] popup open/detect COMET 없을 때 친절 에러
  - [ ] 로딩 인디케이터(텍스트/스피너 중 택1)
  - [ ] 배지: 해당 도메인 워크플로우 존재 시 ON (숫자 옵션)
 - 테스트 기반 작업
  - [ ] Vitest 설정 확인 및 기본 unit 테스트 추가 (domain extractor, cometDom)
  - [ ] mock COMET DOM fixture 연결해 prompt 삽입/submit 테스트
